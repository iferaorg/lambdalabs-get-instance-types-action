...
def main():
    ...
    if response.status_code != 200:
        ...
        print(f'Error: {error["message"]}. Suggestion: {error.get("suggestion", "No suggestion available")}')
        exit(1)

    instance_types = response.json().get('data', {})
    
    if available_only:
        instance_types = {k: v for k, v in instance_types.items() if v.get('regions_with_capacity_available')}
